...
<main>
    <div class="grid grid-cols-1 gap-4 px-4 mt-8 sm:grid-cols-4 sm:px-8">
        <h1 class="text-xl font-bold text-center col-span-4">Edit Dokter</h1>
    </div>
    <div class="container mx-auto">
        <form action="{{ route('dokter.update', $dokter->id) }}" method="POST" class="max-w-md mx-auto">
            @csrf
            @method('PUT')
            ...
            <div class="relative z-0 w-full mb-5 group">
                <input type="text" name="specialization" id="specialization" value="{{ old('specialization', $dokter->specialization) }}" autocomplete="off" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-black dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " required />
