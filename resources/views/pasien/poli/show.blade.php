                                ...
                                <tbody class="bg-white divide-y divide-gray-200">
                                    @forelse($user->pasien->daftarPoli ?? [] as $appointment)
                                        <tr>
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $appointment->dokter->poli->name ?? '-' }}</td>
                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $appointment->dokter->name ?? '-' }}</td>
                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                                {{ \Carbon\Carbon::parse($appointment->schedule->date ?? now())->format('l, d F Y') }}
                                            </td>
                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $appointment->no_antrian }}</td>
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full 
                                                    {{ $appointment->status === 'selesai' ? 'bg-green-100 text-green-800' : 
                                                       ($appointment->status === 'dalam_antrian' ? 'bg-yellow-100 text-yellow-800' : 
                                                       'bg-gray-100 text-gray-800') }}">
                                                    {{ ucfirst(str_replace('_', ' ', $appointment->status ?? 'unknown')) }}
                                                </span>
                                            </td>
                                        </tr>
                                    @empty
